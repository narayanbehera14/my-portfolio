...
// Initialize EmailJS
emailjs.init("b-38tNN6ZfR58r6Of");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const result = await emailjs.sendForm(
        "service_vf3m4hx",           // Your Service ID
        "template_y9e9lkq",         // Your Template ID
        formRef.current
      );
      ...
      const errorText = error?.text || error?.status || "Unknown error";
    ...
    >
      ...
      <div className="max-w-2xl w-full relative z-10 flex flex-col items-center text-center">
        ...
        <form ref={formRef} onSubmit={handleSubmit} className="w-full flex flex-col space-y-6">
          ...
          {/* Submit Button */}
          <div className="animate-element pt-4 flex flex-col items-center gap-4 w-full">
            <button 
              type="submit" 
              disabled={loading}
            ...
            {/* Status Message */}
            {status && (
              <p className={`text-sm text-center ${status.includes("successfully") ? "text-green-400" : "text-red-400"}`}>
                {status}
              </p>
            )}
          </div>
